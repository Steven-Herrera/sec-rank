...
# test webscraper
def test_webscraper():
    rule_URL = "https://www.sec.gov/comments/s7-09-22/s70922.htm"
    scraper = sec_webscraper(rule_URL)
    commenter_names, commenter_links, commenter_dates = scraper.rule_comment_info_retriever()
    assert isinstance(commenter_names, list) and isinstance(commenter_links, list) and isinstance(commenter_dates, list)
...
class sec_webscraper:
    ...
    def rule_comment_info_retriever(self):
        ...
        #split_url = self.rule_URL[:-4].split("/")[-2:]
